...
// ---------- real compiler ----------
// 依赖树住共享池（checkCompilerEnv：包完整 → 依赖树在 → lockfile 漂移三段校验）
let sfc;
{
  const r = checkCompilerEnv();
  if (r.ok) {
    const req = createRequire(join(r.dir, '@vue', 'compiler-sfc', 'package.json'));
...
// icon key truth source: src/assets/icons/*.svg (barrel index.js maps 1:1).
// Missing dir → null → ICONS checks skipped (static pages without icons).
// Key derivation must mirror fetch_icons.mjs safeKey(): camelCase + Icon
// suffix on JS reserved words (package → packageIcon).
...
const iconsDirSrc = join(srcDir, 'assets', 'icons');
const iconKeys = existsSync(iconsDirSrc)
  ? new Set(
      walkFiles(iconsDirSrc, ['.svg'])
        .map((f) => f.slice(iconsDirSrc.length + 1).replace(/\.svg$/, ''))
        .map((kebab) => kebab.replace(/-([a-z0-9])/g, (_, c) => c.toUpperCase()))
        .map((key) => (RESERVED_WORDS.has(key) ? `${key}Icon` : key)),
    )
  : null;
...
for (const file of vueFiles) {
  ...
  // ICONS.key existence: every literal member access ICONS.xxx must exist in
  // src/assets/icons/ (the barrel generated by fetch_icons.mjs maps keys 1:1
  // to svg files). `ICONS[expr]` computed access is statically undecidable —
  // not checked, same responsibility as a real Vite project.
  if (iconKeys) {
    for (const m of source.matchAll(/\bICONS\.([A-Za-z_$][\w$]*)/g)) {
      if (iconKeys.has(m[1])) continue;
      error(`${rel}: uses ICONS.${m[1]} — no such icon key (src/assets/icons/ has: ${[...iconKeys].sort().join(', ')})`);
    }
  }
}
...
for (const file of jsFiles) {
  ...
  // ICONS.key check for .js files (same rule as .vue above); skip the barrel
  // itself — its keys are the truth source by definition
  if (iconKeys && !rel.startsWith('/assets/icons/')) {
    for (const m of text0.matchAll(/\bICONS\.([A-Za-z_$][\w$]*)/g)) {
      if (iconKeys.has(m[1])) continue;
      error(`${rel}: uses ICONS.${m[1]} — no such icon key (src/assets/icons/ has: ${[...iconKeys].sort().join(', ')})`);
    }
  }
