...
本目录是换肤体系的插槽：**每个皮肤一个 css 文件**，默认仅内置 `default.css`。接入自有换肤样式时，把皮肤文件放进本目录即可，无需改动 base.css / bridge.css。

## 皮肤文件协议

1. **命名**：`{skin-name}.css`，`{skin-name}` 为小写英文/数字/连字符（如 `dark.css`、`blue.css`）。
...
3. **注册**：在 `index.gts.html` 的 `<!-- 换肤插槽 -->` 注释处追加一行：
   ```html
   <link rel="stylesheet" href="./src/assets/themes/dark.css">
   ```
4. **切换**：运行时一行代码换肤：
   ```js
...
桥接层（bridge.css）用 `color-mix` 把品牌色与 `--ux-mix-base`（默认白色）混合生成 Element Plus 的 light-N 色阶。**深色皮肤必须**把 `--ux-mix-base` 覆盖为深色表面色（如 `#1d1d1d`），否则按钮 hover/浅色阶会发白。
...
- **改桥接**：把 `bridge.css` 右侧的 `var(--color-...)` 换成自有变量名（桥接层随之脱离 FIXED 约定，需自行维护）。
