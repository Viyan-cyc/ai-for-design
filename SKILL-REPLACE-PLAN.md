...
- **2026-09-12 D23**：**scss 全仓库清零**（资产库 tokens 层 Sass 源退役，纯 CSS 化）。背景：W2-M1 已把 61 组件 .vue 化 + style.scss 内联为 less，全仓库只剩 tokens 层 2 个 .scss（index.scss 入口 + element-plus.scss 编译桥）及其生成模板——而预编译桥 tokens/element-plus.css 早已在位并功能覆盖（--el-* 变量映射运行时 CSS 变量），Sass 编译桥只在 `vite build:library` 编译 index.scss 时被消费，是纯冗余面。改动：build_tokens 删 element-plus.scss.tpl / index.scss.tpl 两模板改产 index.css（平铺 @import 十二层）；库 src/index.ts / main.ts import 改 .css；库 package.json 删 sass devDep、`exports."./tokens"` 改指 index.css（**接口契约变更，须知会设计师侧下游**）；asset-manifest prototypeBaseFiles 同步；init 删 T9 的 scss 剪裁与平铺 index.css 生成段（库直产，拷贝即可）；validate_package 第 6 条改查 index.css 层传播；validate_library 扩展清单去 .scss；顺手删 tokens.json 死变量组 element-plus-6（EP 官方无 --el-alert-title-color/description-color 两变量，V1.3 时代死代码）与 element-plus-dimensions 的 el-compile-* 六值（仅被已删的 scss 编译桥消费）；refresh_release 重锁 401 哈希。验收：`find . -name "*.scss"` 0 命中；build:library 无 sass 通过；validate_package 10/10 + validate_coordination 7/7；init→build 冒烟 OK。
...
- [x] T7 端到端验收：hybrid/free 各生成一页 + 明暗主题 + 毛玻璃 + 换肤 + 二开视角通读（依赖 W2 的 M2 迁移完成，是最终汇合点） (cyc/2026-09-12，详见 tasks/W1-mainline.md 回写：四项实测全过；附带发现 sfc-loader :current-page 单向 prop 渲染 bug 已用 v-model 规避)
